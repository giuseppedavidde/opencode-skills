...
### Step 0 — Auto-update check (skip full rebuild if graph exists)

This step checks what to do before deciding whether to build from scratch or update incrementally.

**Only run this check if the user did NOT pass `--update`, `--cluster-only`, `--query`, `--path`, `--explain`, or `--add`. Those modes already know what they want.**

If `graphify-out/graph.json` does NOT exist → skip this step entirely. Proceed to Step 1 for a full build.

If `graphify-out/graph.json` **does** exist:

Generate the Python interpreter path first (in case the graph exists but `.graphify_python` doesn't):
```bash
...
unchanged = result.get('new_total', 0) == 0 and not deleted
print(json.dumps({'unchanged': unchanged, 'new_total': new_total, 'deleted': len(deleted)}))
"
```

Replace INPUT_PATH with the actual path.

If the JSON says `unchanged: true`:
- Print: "✓ Grafo già aggiornato per `INPUT_PATH`. Usa `graphify query \"<domanda>\"` per esplorare."
- Then immediately offer query/path/explain navigation — non proseguire con Step 1.
- Stop execution here (the graph is fresh, no rebuild needed).

If the JSON says `unchanged: false`:
- Print: "⏫ Rilevati file nuovi/modificati. Avvio update incrementale..."
- Jump directly to the **`--update` workflow** (the section titled "## For --update (incremental re-extraction)"), skipping Steps 1–3 (full extract).
- After the update completes, continue with Steps 4–9 normally.

Replace INPUT_PATH in the command with the actual user-provided path (or `.` if none given).
