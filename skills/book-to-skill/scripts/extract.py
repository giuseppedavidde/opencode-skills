...
def main():
            ...
            try:
                with zipfile.ZipFile(input_path) as zf:
                    ...
                    if "mimetype" in names and zf.read("mimetype").startswith(b"application/epub"):
                        ...
                        print(f"ERROR: Unsupported ZIP-based format '{input_file.name}'. Supported: {supported_formats_message()}", file=sys.stderr)
                        sys.exit(1)
            except (zipfile.BadZipFile, KeyError, OSError):
                print(f"ERROR: Unsupported ZIP-based format '{input_file.name}'. Supported: {supported_formats_message()}", file=sys.stderr)
                sys.exit(1)
        else:
            print(f"ERROR: Unsupported format '{ext or '<none>'}'. Supported: {supported_formats_message()}", file=sys.stderr)
            sys.exit(1)
    ...
    if ext in CALIBRE_EBOOK_EXTENSIONS and not shutil.which("ebook-convert"):
        print("MOBI/AZW/AZW3 extraction requires Calibre's ebook-convert command. Install Calibre and ensure ebook-convert is on PATH, then rerun this command.", file=sys.stderr)
        sys.exit(1)
    ...
    if ext == ".epub":
        ...
        if text is None or not text.strip():
            print(f"ERROR: Could not extract text from {ext}. Install Calibre and ensure ebook-convert is on PATH.", file=sys.stderr)
            sys.exit(1)
        method = "ebook-convert"
        pages = 0
        pages_label = "sections"
    else:
        print(f"ERROR: Unsupported format '{ext}'. Supported: {supported_formats_message()}", file=sys.stderr)
        sys.exit(1)
    ...
    if not structure["has_toc"]:
        print("   WARN    : No table of contents detected — chapter mapping in Step 3 will rely on heading scan only.")
    print(f"\n   Text -> {OUTPUT_TEXT}")
